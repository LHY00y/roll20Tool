document.addEventListener('DOMContentLoaded', () => {
  ...
  async function showEventsPanel(dateStr) {
    ...
    manualEvents.forEach(evt => {
      ...
      li.innerHTML = `
        <span class="evt-item__dot" style="background-color:#1a73e8;"></span>
  ...
  function saveEvent() {
    ...
    if (editingEventId) {
      CalendarEvents.update(editingEventId, { name, date, time, memo, url });
    } else {
      CalendarEvents.add(name, date, time, memo, url);
    }
  ...
  function openIcalDetail(evt) {
    ...
    if (evt.description) infoHtml += `<div class="ical-detail__row"><span class="ical-detail__label">설명</span><span>${escapeHtml(evt.description).replace(/\n/g, '<br>')}</span></div>`;
  ...
  function saveIcalAnnotation() {
    ...
    if (!memo && !url) {
      ...
      CalendarIcal.setAnnotation(icalDetailContext.subId, icalDetailContext.eventUid, memo, url);
    }
  ...
  function renderSubsList() {
    ...
    subs.forEach(sub => {
      ...
      li.innerHTML = `
        <span class="subs-item__dot" style="background-color:${sub.color};"></span>
  ...
  async function addSubscription() {
    ...
    try {
      const sub = CalendarIcal.addSub('iCal', url);
  ...
  function loadSettings() {
    try {
      ...
      if (s) {
        togglePopup.checked = s.popup !== false;
        toggleUrl.checked = s.url !== false;
      }
    } catch { /* 기본값 유지 (둘 다 true) */ }
  }

  function saveSettings() {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({
      popup: togglePopup.checked,
      url: toggleUrl.checked
    }));
  }

  loadSettings();
  togglePopup.addEventListener('change', saveSettings);
  ...
  function checkScheduledEvents() {
    ...
    const popupOn = togglePopup.checked;
